...
awk -v OFS="\t" -v nf=0.1 '{ if(NR==FNR) {c[$1]=$2} else { if($1 in c) {my=c[$1]} else {my=0}; print $1, "sense", $2, my, my * nf }}' ${sense_count} groups.name > ${count}.tmp
awk -v OFS="\t" -v nf=0.1 '{ if(NR==FNR) {c[$1]=$2} else { if($1 in c) {my=c[$1]} else {my=0}; print $1, "antisense", $2, my, my * nf }}' ${antisense_count} groups.name >> ${count}.tmp
