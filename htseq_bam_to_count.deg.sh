...
if [ -f ${STATUS_SORT_F} ]; then
    ...
    samtools sort -n -m 2G -o ${BAM_SORTED} -O bam -T ${RANDOM} -@ ${CPU} ${BAM} && touch ${STATUS_SORT_F}
fi
...
if [ -f ${STATUS_HTSEQ_F} ]; then
    ...
    htseq-count -f bam -r name -s yes -t exon -i ${FEATURE} -m ${HTSEQ_MODE} ${BAM_SORTED} $GTF 2>${LOG} | sort -k1,1 >${OUTPUT}
