...
# Usage ucsc_screenshots.sh 393202989_aSfr1NFH0Tgwt9jhDFJzj3Ng8sBR 1.00
...
ranges=( $(bedtools slop -i ~/data/piPipes/common/dm3/Brennecke.piRNAcluster.bed6.gz -g ~/data/piPipes/common/dm3/dm3.ChromInfo.txt -pct -b ${slop} | cut -f1-4 | grep -v 'X-TAS' | sed -E 's/([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)/\1%3A\2-\3.\4/') )
