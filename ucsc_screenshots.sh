...
# Author: Yu Fu
# Usage ucsc_screenshots.sh hgsid
...
ranges=( $(bedtools slop -i ~/data/piPipes/common/dm3/Brennecke.piRNAcluster.bed6.gz -g ~/data/piPipes/common/dm3/dm3.ChromInfo.txt -b 1000 | cut -f1-4 | grep -v 'X-TAS' | sed -E 's/([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)/\1%3A\2-\3.\4/') )
