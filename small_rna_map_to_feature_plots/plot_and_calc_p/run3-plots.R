...
for(xy in list(list(g1.x, g1.y), list(g2.x, g2.y), list(g3.x, g3.y))) {
    ...
    df.output <- cbind(df.output, s.x, as.x, s.y, as.y, sas.mean, sas.std, p, p.adj)
}

write.table(df.p.fdr, paste(au, ".scatterplots.transposon.cluster.construct.p.fdr.table.txt", sep=""), quote=F, sep="\t", row.names=FALSE)
write.table(df.output, file=paste(au, ".scatterplots.transposon.cluster.construct.table.txt", sep=""), quote=F, sep="\t", row.names=FALSE)
